    ...
    <div className="flex h-screen bg-canvas overflow-hidden">
      ...
      <aside className="w-56 shrink-0 flex flex-col border-r border-border bg-surface">
        ...
        <nav className="flex-1 py-4 px-3 flex flex-col gap-0.5">
          {NAV.map(({ to, label, icon: Icon }) => (
            ...
            >
              {({ isActive }) => (
                <>
                  <Icon size={16} className={isActive ? 'text-green' : 'text-ink-muted group-hover:text-ink-secondary'} />
                  <span className="flex-1">{label}</span>
                  {isActive && <ChevronRight size={12} className="text-green/60" />}
                </>
              )}
            </NavLink>
          ))}
        ...
        <div className="p-4 border-t border-border">
          <p className="text-xs text-ink-muted font-mono">v0.1.0 MVP</p>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 shrink-0 flex items-center px-8 border-b border-border bg-surface/50 backdrop-blur">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-ink-muted font-mono">Supabase conectado</span>
          </div>
        </header>
